...
public:
    ...
    explicit Logger(const std::string &file_name) : file_name_(file_name), queue_(LOG_QUEUE_SIZE){
        ...
        ASSERT(file_.is_open(), std::string(__FILENAME__) +"Could not open log file: " + file_name);
        logger_thread_ = createAndStartThread(-1, "Common/Logger "+ file_name_, [this](){flushQueue();});
        ASSERT(logger_thread_ != nullptr, std::string(__FILENAME__) +"Failed to start Logger thread.");
    }
